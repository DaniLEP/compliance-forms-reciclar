...
export default function TokenList() {
  ...
  const handleSendEmail = async (user) => {
    ...
    // 🔹 URL do FormPage.jsx no outro projeto
    const baseFormURL =
      "https://formulario-complicance-instituto-reciclar-ffjz1k4sy.vercel.app/form";
    ...
    try {
      // 🔹 Marca token como enviado no Firebase
      await set(ref(db, `tokens/${user.token}/sent`), true);
      alert(`Token enviado com sucesso para ${user.email}`);
    } catch (err) {
      console.error(err);
      alert("Erro ao atualizar status do token no Firebase!");
    }
  };
        ...
        <tbody>
          {tokens.map((t) => (
            <tr key={t.token} className="border-b hover:bg-gray-100">
              ...
              <td className="p-2">{t.email}</td>
              <td className="p-2 font-mono text-sm">{t.token}</td>
              <td className="p-2">{t.used ? "✅ Preenchido" : "⏳ Não Preenchido"}</td>
              <td className="p-2">
                {!t.sent ? (
                  <button
                    onClick={() => handleSendEmail(t)}
